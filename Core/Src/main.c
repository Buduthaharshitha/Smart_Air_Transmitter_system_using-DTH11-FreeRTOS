...
void TimerTask(TimerHandle_t TimerHandle);
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
int _write(int file,char *ptr,int len)//to display
{
...
void ReceiverTask(void *arg)
{
	...
	uint8_t hum_buf;
		for(;;){
			xQueueReceive(QueueHandle,&temp_buf, portMAX_DELAY);

			xQueueReceive(QueueHandle,&hum_buf,portMAX_DELAY);
